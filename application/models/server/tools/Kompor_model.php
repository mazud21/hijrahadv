...
class Kompor_model extends CI_model {
    public function getAllKompor(){
        $this->db->select('*');
        $this->db->from('tools t');
        $this->db->join('jenis j','j.id_jenis=t.id_jenis');
        $this->db->where('j.desc_','Kompor Portabel');
    ...
    public function tambahDataKompor($upload){
        $data = array(
            'nama'       => $this->input->post('nama'),
            'merk'       => $this->input->post('merk'),
    ...
    public function hapusDataKompor($id_tool){
        // $this->db->where('id', $id);
    ...
    public function cariDataKompor(){
